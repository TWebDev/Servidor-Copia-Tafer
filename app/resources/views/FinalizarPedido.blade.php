              ...
              <div class="box-footer d-flex flex-wrap align-items-center justify-content-between">
                <div class="left-col"><a href="{{ url()->previous() }}" class="btn btn-secondary mt-0"><i class="fa fa-chevron-left"></i>Regresar</a></div>
                  <div class="right-col">
                    ...
                    <form action="{{ route('PagoSucursal')}}" method="post">
                    @csrf
                    <input type="hidden" name="direccion" value="{{\Session::get('direccion')}}">
                    <input type="hidden" name="metodopago" value="{{\Session::get('metodopago')}}">
                    <input type="hidden" name="metodoentrega" value="{{\Session::get('metodoentrega')}}">
                      <button type="submit" class="btn btn-primary" data-toggle="modal" data-target="#myModal">
                        <a type="submit" class="btn btn-template-main">Finalizar Pedido<i class="fa fa-chevron-right"></i></a>
                      </button>
                    </form>
                    @elseif($metodopago=='Oxxo')
                    <form action="{{ route('PagoOxxo')}}" method="post">
                    @csrf
                    <input type="hidden" name="direccion" value="{{\Session::get('direccion')}}">
                    <input type="hidden" name="metodopago" value="{{\Session::get('metodopago')}}">
                    <input type="hidden" name="metodoentrega" value="{{\Session::get('metodoentrega')}}">
                    ...
                    <form action="{{ route('PagarPaypal')}}" method="post">
                    @csrf
                    <input type="hidden" name="direccion" value="{{\Session::get('direccion')}}">
                    <input type="hidden" name="metodopago" value="{{\Session::get('metodopago')}}">
                    <input type="hidden" name="metodoentrega" value="{{\Session::get('metodoentrega')}}">
