  ...
  <body>
    <div id="all">
      <div class="admin-bar">
        ...
        <a href="{{route('vistaventas')}}" class="icon icon-coin-dollar" id="icon2"></a>
    ...
    <header class="nav-holder make-sticky">
      ...
      <div id="content">
        <div class="container">
          <div class="row bar" style="padding-bottom: 193px">
                ...
                <div class="table-responsive">
                  <table class="table">
                    ...
                    <tbody>
                      ...
                      <tr>
                        <td><a href="{{route('vistaventas',$item ->id)}}">{{$item -> id}}</a></td>
                        <td>{{$item->total}}</td>
                        <td>{{ $item -> nombre}}</td>
                        <td>{{$item -> direccion}}</td>
                        @if($item -> status=='Pagado')
                          <td><span class="badge badge-primary">{{$item -> status}}</span></td>
                        @elseif($item -> status=='Pendiente de pago')
                          <td><span class="badge badge-warning">{{$item -> status}}</span></td>
                        @elseif($item -> status=='Entregado')
                         <td> <span class="badge badge-success">{{$item -> status}}</span></td>
                        @endif
                        <td>{{$item->metodopago}}</td>
                        <td><a href="{{route ('actpago', $item -> id)}}"><i class="fa fa-money"></i></a></td>
                        <td><a href="{{route ('entrega', $item -> id)}}"><i class="fa fa-check"></i></a></td>
                      </tr>
                      @endforeach
                    </tbody>
                  </table>
      ...
      function drawStuff() {
        var data = new google.visualization.arrayToDataTable([
          ['Move', 'Productos Vendidos '],
            @foreach ($RVentas as $tablas)
        ...
        var options = {
          ...
          chart: {
            title: 'Que productos son los más Vendidos',
            subtitle: '' },
          axes: {
            x: {
    ...
    <div id="top_x_div" style="width: 800px; height: 600px;padding-left: 500px; padding-bottom: 50px"></div>
            <!-- Empieza Footer -->
